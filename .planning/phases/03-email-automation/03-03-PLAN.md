...
user_setup:
  - service: netlify
    ...
    env_vars:
      ...
      - name: KIT_TAG_ID_BUYER
        source: "Kit API: GET https://api.convertkit.com/v3/tags?api_key=YOUR_KEY → find 'buyer' tag ID"
...
must_haves:
  truths:
    ...
    - "All buyers receive the generic 'buyer' tag in Kit (in addition to the product-specific tag)"
...
6. **Tag buyer in Kit (generic "buyer" tag):**
   - After the product-specific tag succeeds, make a SECOND POST to apply the generic buyer tag
   - POST to `https://api.convertkit.com/v3/tags/${process.env.KIT_TAG_ID_BUYER}/subscribe`
   - Body: `{ "api_secret": process.env.KIT_API_SECRET, "email": buyerEmail }`
   - Content-Type: application/json
   - If this call fails, log the error but still return 200 (the product-specific tag is the critical one; the generic tag is supplementary)
   - Return 200 "OK" on success of both calls

7. **Add CORS headers** to allow Netlify's health checks:
...
- Log the buyer email and product for debugging (Netlify logs are private)
  ...
  <verify>
Files exist: `webhook/netlify/functions/ls-webhook.js` and `webhook/netlify.toml`. The function file contains: crypto import, signature verification using timingSafeEqual, event_name check, product-to-tag mapping, TWO Kit API POST calls (product-specific tag + generic "buyer" tag using KIT_TAG_ID_BUYER). No hardcoded secrets or IDs. The netlify.toml points functions to the correct directory.
  </verify>
  <done>
Netlify Function code is complete with HMAC signature verification, product-to-tag routing, generic "buyer" tagging, and Kit API integration — ready for deployment.
  </done>
</task>
...
**PREFLIGHT: Verify Netlify Free Tier Pricing**
Netlify transitioned to credit-based pricing after September 2025. Before creating an account:
1. Check https://www.netlify.com/pricing/ to confirm the free tier still supports serverless functions
2. If the free tier is too restrictive or requires a credit card, use **Cloudflare Workers** as a fallback:
   - Cloudflare Workers free tier: 100,000 requests/day (more than sufficient)
   - Sign up at https://dash.cloudflare.com/ → Workers & Pages → Create Worker
   - The ls-webhook.js code needs minimal adaptation (change `exports.handler` to `export default { async fetch(request) { ... } }`)
   - Deploy via Wrangler CLI: `npx wrangler deploy`
   - The function URL format changes to: `https://ls-webhook.<your-subdomain>.workers.dev`
3. If you proceed with Cloudflare Workers instead, adjust all "Netlify" references in the steps below accordingly
...
2. **Kit Tag IDs:**
   ...
   ```
   - Find the IDs for: `purchased-starter`, `purchased-full-kit`, and `buyer`
...
| KIT_TAG_ID_BUYER | (from step A.2 — find the 'buyer' tag ID) |
...
4. All 7 environment variables are set in Netlify (including KIT_TAG_ID_BUYER)
...
- A Lemon Squeezy order_created event results in both the product-specific tag AND the generic "buyer" tag being applied in Kit
